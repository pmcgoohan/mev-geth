...
// SendPrivateTransactionArgs represents the arguments for a SendPrivateTransaction call.
...
// SendBundle will add the signed transaction to the transaction pool.
// The sender is responsible for signing the transaction and using the correct nonce and ensuring validity
func (s *PrivateTxBundleAPI) SendPrivateTransaction(ctx context.Context, args SendPrivateTransactionArgs) error {
	if len(args.Tx) == 0 {
		return errors.New("missing tx")
	}
	var blockNum uint64
	if args.MaxBlockNumber == nil {
		_, parent, err := s.b.StateAndHeaderByNumber(ctx, rpc.LatestBlockNumber)
		if err != nil {
			return err
		}
		blockNum = parent.Number.Uint64() + 25
	} else {
		blockNum = *args.MaxBlockNumber
	}
	...
	return s.b.AddPrivateTransaction(ctx, tx, blockNum)
}
