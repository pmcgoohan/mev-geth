...
func newMultiWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool, init bool) *multiWorker {
	db, err := database.NewDatabaseService(os.Getenv("FLASHBOTS_POSTGRES_DSN"))
	if err != nil {
		log.Error("FLASHBOTS_POSTGRES_DSN env variable not set")
	}

	queue := make(chan *task)

	regularWorker := newWorker(config, chainConfig, engine, eth, mux, isLocalBlock, init, db, &flashbotsData{
		isFlashbots: false,
		queue:       queue,
	})
	...
	for i := 1; i <= config.MaxMergedBundles; i++ {
		workers = append(workers,
			newWorker(config, chainConfig, engine, eth, mux, isLocalBlock, init, db, &flashbotsData{
				isFlashbots:        true,
