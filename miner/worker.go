...
import (
	...
	"github.com/ethereum/go-ethereum/flashbotsextra/database"
...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool, init bool, db *database.DatabaseService, flashbots *flashbotsData) *worker {
		...
		case ev := <-w.chainSideCh:
			...
			if w.isRunning() && w.current != nil && w.current.uncles.Cardinality() < 2 {
				...
				if err := w.commitUncle(w.current, ev.Block.Header()); err == nil {
					...
					w.commit(uncles, nil, true, start, new(big.Int), nil)
				}
			}
...
func (w *worker) taskLoop() {
	...
	for {
		...
		case task := <-w.taskCh:
			...
			taskParentHash := header.ParentHash
			...
			if len(task.bundle) > 0 && w.db != nil {
				profit := new(big.Rat).SetFrac(task.profit, big.NewInt(params.Ether))
				bundleProfit := new(big.Rat).SetFrac(task.bundleProfit, big.NewInt(params.Ether))
				err := w.db.AddMegabundle(header.Number.Int64(), header.ParentHash.Hex(), profit, bundleProfit, task.worker, task.processingTimeMicros, task.bundle, task.revertingTxHashes)
				if err != nil {
					log.Error("error adding megabundle to DB", "err", err)
				}
			}
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	if !noempty && atomic.LoadUint32(&w.noempty) == 0 {
		w.commit(uncles, nil, false, tstart, new(big.Int), nil)
	}

	// Fill the block with all available pending transactions or bundles.
	...
	if w.flashbots.isFlashbots && !w.flashbots.isMegabundleWorker {
		...
		privateTxs, err := w.eth.TxPool().GetPrivateTransactions(header.Number.Uint64())
		if err != nil {
			log.Error("Failed to fetch pending private transactions", "err", err)
			return
		}
		bundleTxs, bundle, numBundles, err := w.generateFlashbotsBundle(bundles, privateTxs, w.coinbase, parent, header, pending)
		if err != nil {
		...
		w.current.bundle = bundleTxs
		w.current.profit.Add(w.current.profit, bundle.ethSentToCoinbase)

		bundleProfit.Set(w.current.profit)
		revertingTxHashes = bundle.originalBundle.RevertingTxHashes
	}
	if w.flashbots.isMegabundleWorker {
		...
		bundleProfit.Set(coinbaseDelta)
		revertingTxHashes = megabundle.RevertingTxHashes
	...
	w.commit(uncles, w.fullTaskHook, true, tstart, bundleProfit, revertingTxHashes)
}

// commit runs any post-transaction state modifications, assembles the final block
// and commits new work if consensus engine is running.
func (w *worker) commit(uncles []*types.Header, interval func(), update bool, start time.Time, bundleProfit *big.Int, revertingTxHashes []common.Hash) error {
	...
	if w.isRunning() {
		...
		select {
		case w.taskCh <- &task{receipts: receipts, state: s, block: block, createdAt: time.Now(), profit: w.current.profit, isFlashbots: w.flashbots.isFlashbots, worker: w.flashbots.maxMergedBundles, isMegabundle: w.flashbots.isMegabundleWorker, bundle: w.current.bundle, processingTimeMicros: time.Since(start).Microseconds(), bundleProfit: bundleProfit, revertingTxHashes: revertingTxHashes}:
...
func transformPrivateTxsToBundles(privateTxs []*types.Transaction, baseFee *big.Int) []simulatedBundle {
	bundles := []simulatedBundle{}
	for _, tx := range privateTxs {
		tip, err := tx.EffectiveGasTip(baseFee)
		if err != nil {
			continue
		}
		bundles = append(bundles, simulatedBundle{
			mevGasPrice:    tip,
			originalBundle: types.MevBundle{Txs: []*types.Transaction{tx}}, // TODO: add in revertingTxHashes? unclear why we would want to
		})
	}
	return bundles
}

func (w *worker) generateFlashbotsBundle(bundles []types.MevBundle, privateTxs []*types.Transaction, coinbase common.Address, parent *types.Block, header *types.Header, pendingTxs map[common.Address]types.Transactions) (types.Transactions, simulatedBundle, int, error) {
	simulatedBundles, err := w.simulateBundles(bundles, coinbase, parent, header, pendingTxs)
	if err != nil {
		return nil, simulatedBundle{}, 0, err
	}

	privateBundles := transformPrivateTxsToBundles(privateTxs, header.BaseFee)
	simulatedBundles = append(simulatedBundles, privateBundles...)
...
func (w *worker) mergeBundles(bundles []simulatedBundle, parent *types.Block, header *types.Header, pendingTxs map[common.Address]types.Transactions) (types.Transactions, simulatedBundle, int, error) {
	...
	for _, bundle := range bundles {
		...
		log.Info("simming bundle for merging", "tx[0]", bundle.originalBundle.Txs[0].Hash().Hex(), "mevGasPrice", bundle.mevGasPrice)
		...
		if err != nil || simmed.mevGasPrice.Cmp(floorGasPrice) <= 0 {
			log.Info("error computing bundle gas", "tx[0]", bundle.originalBundle.Txs[0].Hash().Hex(), "err", err)
			state = prevState
			gasPool = prevGasPool
			continue
		}
		...
		revertingTxHashes = append(revertingTxHashes, bundle.originalBundle.RevertingTxHashes...)
		count++
	...
	return finalBundle, simulatedBundle{
		...
		originalBundle:    types.MevBundle{Txs: finalBundle, RevertingTxHashes: revertingTxHashes},
	}, count, nil
}
