...
type TxPool struct {
	...
	privateTxs  map[uint64]types.Transactions      // Private txs, mapped to the largest blockNumber allowed
...
func NewTxPool(config TxPoolConfig, chainconfig *params.ChainConfig, chain blockChain) *TxPool {
	...
	pool := &TxPool{
		...
		privateTxs:      make(map[uint64]types.Transactions),
	}
...
// AddPrivateTransaction adds a private tx to the pool
func (pool *TxPool) AddPrivateTransaction(tx *types.Transaction, maxBlockNumber uint64) error {
	pool.mu.Lock()
	defer pool.mu.Unlock()

	txs := pool.privateTxs[maxBlockNumber]

	pool.privateTxs[maxBlockNumber] = append(txs, tx)
	log.Info("added private tx", "tx", tx.Hash().Hex(), "maxBlockNumber", maxBlockNumber)

	return nil
}

func (pool *TxPool) GetPrivateTransactions(currentBlockNumber uint64) (types.Transactions, error) {
	pool.mu.Lock()
	defer pool.mu.Unlock()

	txs := []*types.Transaction{}
	for maxBlockNum, blockTxs := range pool.privateTxs {
		// cleanup first
		if maxBlockNum+2 < currentBlockNumber { // keep last few blocks around for re-orgs
			delete(pool.privateTxs, maxBlockNum)
		}

		if maxBlockNum >= currentBlockNumber {
